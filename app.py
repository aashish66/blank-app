...
def get_scale_for_area(aoi=None, buffer_km=10, sensor="Sentinel-2"):
    ...
    Larger areas need coarser resolution to avoid computation limits.
    """
    # Calculate area
    area_sqkm = None
    if aoi is not None:
...
if page == "🛰️ Satellite Analysis":
    ...
    if aoi is not None:
        # Calculate scale based on method
        if aoi_method == "📍 Coordinates + Buffer":
            current_buffer = buffer_km
        else:
            current_buffer = 20  # Default for non-buffer methods
        
        if current_buffer > 40:
            st.warning(f"⚠️ Large area ({current_buffer}km). Visualization may be coarser.")
        ...
        with st.spinner("Generating vegetation map..."):
            try:
                ...
                if vmin_raw is None or vmax_raw is None:
                    st.warning("""⚠️ **No data found for this area!** Try:
                    - Use **Median Composite** mode
                    - Reduce area size or switch to **MODIS** sensor
                    """)
                    vmin = -0.2
                    vmax = 0.8
                else:
                ...
                # Visualization with explicit bands parameter
                vis_params = {
                    'bands': [band_name],
                    'min': vmin, 
                    'max': vmax,
                ...
                # Add outline-only AOI boundary (no fill)
                empty = ee.Image().byte()
        ...
        if aoi and ('compare_img1_type' in st.session_state and 'compare_img2_type' in st.session_state):
            with st.spinner("Generating comparison maps..."):
                try:
                    ...
                    m3.addLayer(aoi, {'color': 'blue'}, 'AOI')
                    m3.centerObject(aoi)
